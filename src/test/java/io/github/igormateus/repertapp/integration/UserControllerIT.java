...
class UserControllerIT {
    ...
    @Test
    @DisplayName("whoami returns userResponse authenticated when sucessful")
    void whoami_returnsUserResponseAuthenticated_whenSucessful() {
        AppUser validUser = UserCreator.createValid();
        UserAuthDTO userAuthDTO = UserAuthDTOCreator.createValid();
        userService.signup(validUser);
        String token = userService.signin(userAuthDTO.getUsername(), userAuthDTO.getPassword());

        HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.AUTHORIZATION, "Bearer " + token);
                        
        ResponseEntity<UserResponseDTO> authenticationResponse = testRestTemplate.exchange(
            "/users/me",
			HttpMethod.GET,
        ...
        Assertions.assertThat(authenticationResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
        Assertions.assertThat(authenticationResponse.getBody()).isNotNull();
        Assertions.assertThat(authenticationResponse.getBody().getId()).isNotNull();
        Assertions.assertThat(authenticationResponse.getBody().getCreatedAt()).isNotNull();
        Assertions.assertThat(authenticationResponse.getBody().getBio()).isEqualTo(validUser.getBio());
        Assertions.assertThat(authenticationResponse.getBody().getEmail()).isEqualTo(validUser.getEmail());
        Assertions.assertThat(authenticationResponse.getBody().getName()).isEqualTo(validUser.getName());
        Assertions.assertThat(authenticationResponse.getBody().getUsername()).isEqualTo(validUser.getUsername());
    }

    @Test
    @DisplayName("whoami returns forbidden when token is invalid")
    void whoami_returnsForbidden_whenTokenIsInvalid() {
        AppUser validUser = UserCreator.createValid();
        userService.signup(validUser);
        ...
        Assertions.assertThat(authenticationResponse).isNotNull();
        Assertions.assertThat(authenticationResponse.getStatusCode()).isEqualTo(HttpStatus.FORBIDDEN);
        Assertions.assertThat(authenticationResponse.getBody()).isNull();
    }

    // editMe_returnsUserResponseUpdated_whenSucessful

    
    // editMe_returnsForbidden_whenInvalidToken
    // editMe_returnsAnError_whenInvalidUsernameWasSent
    // editMe_returnsAnError_whenNullPasswordWasSent
    // editMe_returnsAnError_whenInvalidNameWasSent
    // editMe_returnsAnError_whenInvalidEmailWasSent
    // editMe_returnsAnError_whenInvalidBioWasSent
    // editMe_returnsAnError_whenUsernameAlreadyInUse
    // editMe_returnsAnError_whenNameAlreadyInUse
    // editMe_returnsAnError_whenEmailAlreadyInUse
